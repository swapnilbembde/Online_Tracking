...
from models.reid.inception import Inception_v1
from models.reid.cbp import CompactBilinearPooling as pooling 
...
class Body(nn.Module):
    ...
    def forward(self, inputs):
        ## similar to resnet structure and pose extractor from CMU-openpose
    ...
    def init_pretrained(self):
        ...
        state_dict = torch.load('/home/SharedData/swapnil/tracker/re-id/reid/pretrained/pose_iter_440000.caffemodel.pth')
        ...
        print('appearance_body pretrained model loaded!')
...
class Bilinear_Pooling(nn.Module):
    ...
    def __init__(self, num_feat1=512, num_feat2=128, num_feat_out=512):
        ...
        self.cbp = pooling(self.num_feat1, self.num_feat2, self.num_feat_out, sum_pool=True)
...
class Appearance_Body(nn.Module):
    def __init__(self, num_features=512, use_bn=True, use_relu=False, dilation=1, initialize=True):
        ...
        self.part_feat_extractor = Body(1, num_features_part=num_features_part, dilation=dilation, initialize=initialize)
    ...
    def forward(self, inputs):
        ...
        outp = self.pooling(output_app, output_part)
        #print( outp.shape,outp)
        return outp
...
def appearance_body_map(features=512, use_relu=False, dilation=1, initialize=True):
    model = Appearance_Body(num_features=features, use_relu=use_relu, dilation=dilation, initialize=initialize)

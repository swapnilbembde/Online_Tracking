...
from models.tracker import track_utils
...
from .trackstate import TrackNumbered, TrackState


class baseTrack(TrackState):
    ...
    def predict(self):
        ...
        if self.state != TrackNumbered.Tracked:
            mean_state[7] = 0
    ...
    def activate(self, kalman_filter, frame_id, image):
        ...
        self.time_by_tracking = 0
        self.tracklet_len = 0
        self.state = TrackNumbered.Tracked
...
class OnlineTracker(object):
    ...
    def __init__(self, min_cls_score=0.4, min_ap_dist=0.64, max_time_lost=30, use_tracking=True, use_refind=True):
        ...
        self.tracked_stracks = []   # type: list[baseTrack]
        self.lost_tracks = []      # type: list[baseTrack]
        self.removed_tracks = []   # type: list[baseTrack]
    ...
    def update(self, image, tlwhs, det_scores=None):
        ...
        """step 1: prediction"""
        for strack in itertools.chain(self.tracked_stracks, self.lost_tracks):
            strack.predict()

        """step 2: scoring and selection"""
        if det_scores is None:
            det_scores = np.ones(len(tlwhs), dtype=float)
        detections = [baseTrack(tlwh, score, from_det=True) for tlwh, score in zip(tlwhs, det_scores)]
        ...
        if self.classifier is None: 
            ...
            if self.use_tracking:
                tracks = [baseTrack(t.tlwh, t.tracklet_score(), from_det=False)
                          for t in itertools.chain(self.tracked_stracks, self.lost_tracks) if t.is_activated]
        ...
        unconfirmed = []
        tracked_stracks = []  # type: list[baseTrack]
        ...
        dists = track_utils.nearest_reid_distance(tracked_stracks, detections, metric='euclidean')
        #logger.debug('distance matrix:{}'.format(dists))
        dists = track_utils.build_cost_matrix(self.kalman_filter, dists, tracked_stracks, detections)
        matches, u_track, u_detection = track_utils.linear_assignment(dists, thresh=self.min_ap_dist)
        for itracked, idet in matches:
            tracked_stracks[itracked].update(detections[idet], self.frame_id, image)

        # matching for missing targets
        detections = [detections[i] for i in u_detection]
        dists = track_utils.nearest_reid_distance(self.lost_tracks, detections, metric='euclidean')
        dists = track_utils.build_cost_matrix(self.kalman_filter, dists, self.lost_tracks, detections)
        matches, u_lost, u_detection = track_utils.linear_assignment(dists, thresh=self.min_ap_dist)
        for ilost, idet in matches:
            track = self.lost_tracks[ilost]  # type: baseTrack
            det = detections[idet]
        ...
        dists = track_utils.iou_distance(r_tracked_stracks, detections)
        matches, u_track, u_detection = track_utils.linear_assignment(dists, thresh=0.7)
        for itracked, idet in matches:
        ...
        dists = track_utils.iou_distance(unconfirmed, detections)
        matches, u_unconfirmed, u_detection = track_utils.linear_assignment(dists, thresh=0.7)
        for itracked, idet in matches:
        ...
        self.tracked_stracks = [t for t in self.tracked_stracks if t.state == TrackNumbered.Tracked]
        self.lost_tracks = [t for t in self.lost_tracks if t.state == TrackNumbered.Lost]  # type: list[baseTrack]
        self.tracked_stracks.extend(activated_tracks)
        self.tracked_stracks.extend(refind_tracks)
        self.lost_tracks.extend(lost_tracks)
        self.removed_tracks.extend(removed_tracks)



        rois = np.asarray([t.tlbr for t in self.lost_tracks], dtype=np.float32)
        lost_cls_scores = self.classifier.predict(rois)
        out_lost_stracks = [t for i, t in enumerate(self.lost_tracks)
        ...
        logger.debug('Activated: {}'.format([track.track_id for track in activated_tracks]))
        logger.debug('Refind: {}'.format([track.track_id for track in refind_tracks]))
        logger.debug('Lost: {}'.format([track.track_id for track in lost_tracks]))
        logger.debug('Removed: {}'.format([track.track_id for track in removed_tracks]))        

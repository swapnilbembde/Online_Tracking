...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., ids2=None):
    ...
    text_scale = max(1, image.shape[1] / 600.)	## original setting 1/800 , for live demo 1/200
    text_thickness = 2 #if text_scale > 1.1 else 1
    line_thickness = max(1, int(image.shape[1] / 200.)) ## original setting 1/800 , for live demo 1/200
...
def plot_trajectory(image, tlwhs, track_ids):
    image = image.copy()
    #print(tlwhs,len(tlwhs))
    for one_tlwhs, track_id in zip(tlwhs, track_ids):
        color = get_color(int(track_id))
        #print(one_tlwhs,len(one_tlwhs))
